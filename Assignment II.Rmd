...
```{r}
str(df.energy)
df.energy <- df.energy %>% select(-date)
```
```{r}
summary(df.energy)
```
...
energy.model.1 <- lm(Appliances ~ lights + RH_out + T2 + T6 + T_out + RH_8 + Windspeed + T3 + RH_6 + RH_1 + RH_2 + T1 + RH_7 + RH_9, data = train.energy)
...
energy.model.2 <- lm(Appliances ~ lights + RH_out + T2 + T6 + T_out + RH_8 + Windspeed + T3 + RH_6 + RH_1 + RH_2 + T1 + RH_7 + RH_9, data = train.energy.log)
...
```{r}
best.subset <- regsubsets(Appliances ~ lights + RH_out + T2 + T6 + T_out + RH_8 + Windspeed + T3 + RH_6 + RH_1 + RH_2 + T1 + RH_7 + RH_9, data = train.energy.log)
...
```
```{r}
energy.model.3 <- lm(Appliances ~ lights + T2 + RH_8 + Windspeed + T3 + RH_1 + RH_2 + RH_9, data = train.energy.log)
summary(energy.model.3)
```
```{r}
# plot Q-Q plot 
qqnorm(energy.model.3$residuals, main = 'Q-Q Plot of Residual')
qqline(energy.model.2$residuals)
```
